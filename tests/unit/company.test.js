...
describe('company.js', () => {
  ...
  describe('getCompanyData (no cache)', () => {
    it('should fetch Continental Hotels via direct CIF lookup and return company data', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse(CONTINENTAL_ANAF_RECORD));
    ...
    it('should throw when ANAF returns no company name', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse({ cui: 1559737, name: null }));
  ...
  describe('validateAndGetCompany', () => {
    ...
    if (CONTINENTAL_ANAF_RECORD.inactive) {
