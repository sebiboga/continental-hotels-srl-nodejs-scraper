...
| `src/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by both `validate-jobs.js` and `tests/validate-continental-jobs.js`. |
...
| `tests/company.json` | Mock ANAF company data for Continental Hotels used in unit tests |
| `tests/validate-continental-jobs.js` | **Continental Hotels-specific fast validator (used by CI).** HEAD requests only, hardcoded Continental Hotels CIF. Called nightly by `automation-testing.yml`. Supports `--dry-run` and `--delete`. |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real Continental Hotels API, ANAF, and SOLR |
...
| `ROBOTS.md` | robots.txt analysis and scraping policy for Continental Hotels Careers |
...
| `delete_request.json` | **Manual maintenance tool** — SOLR payload to delete ALL jobs for CIF 1559737. Use only when you need to wipe Continental Hotels jobs from SOLR entirely. Run with: `curl --user "${SOLR_AUTH}" "https://solr.peviitor.ro/solr/job/update?commit=true" -H "Content-Type: application/json" -d @delete_request.json` |
...
- Full workflow: validate company (ANAF+Peviitor) → scrape Continental Hotels → transform → upsert SOLR → generate docs/jobs.md