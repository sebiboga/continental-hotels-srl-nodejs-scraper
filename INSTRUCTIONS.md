...
This scraper extracts job listings from Continental Hotels careers page (Romania only) and imports them to peviitor.ro.

Target: https://www.jobs-continentalhotels.ro/en/jobs/romania
...
1. **Start with brand** - We know the brand (e.g., "Continental Hotels")
...
8. **Scrape new jobs** - Extract jobs from Continental Hotels careers page (Romania)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts Continental Hotels Careers jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from Continental Hotels careers API (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape Continental Hotels API (jobs for Romania)
    │
    ▼
...
| `tests/validate-continental-jobs.js` | CI fast validator (HEAD only); thin CLI over `src/job-validator.js` + `solr.js` |
...
| `tests/e2e/scraper.test.js` | End-to-end tests with real Continental Hotels API |
